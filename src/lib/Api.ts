...
export const signup = async (data: {
    ...
    try {
      const response = await axiosInstance.post("/api/v1/auth/account/signup", data);
...
export const login = async (data: {email: string; password: string}) => {
   try{ const response = await axiosInstance.post ("/api/v1/auth/account/login", data);
...
export const forgetPassword = async ( data: { email: string }) => {
    const response = await axiosInstance.post ("/api/v1/auth/account/forgot-password/", data);
    return response. data;
}
